...
At some point I need to combine all behaviour words together.
I tried having the top articles be the top 50% and only 5 words showed up: dog, dogs, human, study, results.
If I look at a plot of unique words, what is the distribution?
...
Some words are the same but different tense, plural, etc. So I will need to standardise the words to compare them effectively.

1. Using the list of words, find common words like behaviour, behaviours, behavior, behavioural, etc.
Then do a find an replace at the abstract level to group words that mean the same thing.
2. document in overleaf

After looking through the list, a lot of words have alternatives that can be used. So to reduce the labor and decrease subjectivity, we are using stemming software to get to the stem of words and then do the frequency analysis.
...
### New angle: getting to stem words
In NLP Stemming is a tool that cuts words down to their stems. This 
...
abs_word_relate <- toparticles_wordstems %>% 
  ...
  pairwise_cor(item=stem, feature = ArticleCount)
```
