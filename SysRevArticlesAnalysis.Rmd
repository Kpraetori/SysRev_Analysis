...
#    str_detect(locations, "Department of Psychiatry and Behavioral Sciences University of Arkansas for Medical Sciences 4301 West Markham Little Rock Arkansas 72205") ~ " Department of Psychiatry and Behavioral Sciences University of Arkansas for Medical Sciences 4301 West Markham Little Rock Arkansas 72205 United States",
#      str_detect(locations, "Ross University School of veterinary Medicine Basseterre St Kitts Saint Kitts and Nevis     Adtalem Global Education Downers Grove Illinois United Kingdom     Center for Teaching and Learning Ross University School of Medicine Roseau Commonwealth of Dominica Dominica") ~ "Saint Kitts and Nevis St Kitts and Nevis KNA KN",
...
```{r}
locationtwo %>%
  filter(str_detect(country, 'nevis'))
```

Alright so there is something wrong with string detect as it doesn't like replacing korea. I will sort this later.
Also I need to check that all the replacements are accurate to first author location. I'll manually compare the columns before and after the changes.
...
After manual inspection of the entire dataset, some codes were not correct to the first author affiliated country. Here is a manual correction of the affected country codes.
```{r}
...
### TODO: if stuff gives a problem check the format of the country names

# Join the datasets
mapdata <- left_join(mapdata, aggcountry, by= c("region" = "country"))

# filter for only countries with entries
mapdatayes <- mapdata %>% filter(!is.na(mapdata$n))
mapdatayes
...
# Check the dataset
mapdata %>%
  filter(str_detect(region, 'United Kingdom'))
```

```{r}
# globemap1 <- ggplot(mapdata, aes(x=long, y=lat, group=group)) +
#   geom_polygon(aes=(fill=mapdata$n)) # this is wrong

globemap2 <- ggplot() + 
  geom_polygon(data=mapdata, aes(fill=n, x=long, y=lat, group=group)) +
  scale_fill_gradient(name="Count of articles", low="aquamarine3", high="turquoise4", na.value="snow3") +
  theme_void() +
  coord_map()
globemap2
...
globemap2 <- ggplot() + 
  geom_polygon(data=mapdata, aes(fill=n, x=long, y=lat, group=group)) +
  scale_fill_viridis(name="Number of articles", option="G", na.value="snow") +
  labs(
    title = "TITLE",
    subtitle = "SUBTITLE",
    caption = "CAPTION"
  ...
  theme(
    ...
    plot.background = element_rect(fill = "snow3", color = NA),
    panel.background = element_rect(fill = "snow3", color = NA),
    legend.background = element_rect(fill = "snow3", color = NA),
