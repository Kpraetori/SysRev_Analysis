...
This code is intended to clean, prepare, and analyse systematic review
data from the abstract and title screening stage. These steps may be
broken up into separate files within the project for clarity.

## Data Analysis

From the published methods section: Descriptive statistics of articles
screened at the title and abstract level was used to identify gross
trends in the literature and to explore methodology. Descriptive
statistics included: X publication trend of number of papers by date and
by journal, X number of papers by first author location, if listed.

Text mining was used to pull relevant words from abstracts to explore: X
frequency of word (how many articles contain x words), X word frequency
by year (overall frequency) X with correlation tests (overall frequency)
XNOT USED, not helpful to do analysis given scientific writing structure
sentiment analysis X visualising a network of words used together and a
correlation of words including X a visualisation of correlations within
word clusters.

Text mining was conducted using tidytext package in R \citep{Silge2016}.
From the list of relevant words, methodology-based words were isolated
and counted for frequency. For example, words like behaviour
observation, preference test, cortisol, oxytocin, and other such
measures pertaining to affect or welfare.

The top 3 most-frequently mentioned method will be used as the selection
criterion for full-text screening and appraisal, to not exceed 20%
(n=62) of all considered papers.

#### Quick tips

Cmd + Option + I for a new chunk. 
ctl + shift + M for new pipe.
...
### Code for citing current packages
...
articles.csv aka rawdata is the file exported straight from Rayyan
...
```{r}
spec(rawdata) # Look at the file
```

The file contains included and excluded articles. I just want to select
those that have been included, so I'll filter for strings containing
RAYYAN-INCLUSION: {"Kelsey"=\>"Included" or pass new rule (does that
apply to all included though?)

When I filtered for 'Included', the number matches the number included
in our study. Woo!
...
Before doing anything more, let's look to see if there are important
missing values.
...
It looks like location is missing for a lot of papers, so that's going
to need fixing before we can look at location data. The year is missing
for one paper. We should isolate that and find out what year it is
before moving on. All the papers have titles and abstracts and journals,
and only 3 are missing keywords. This is good. Some are missing DOI and
URL. This is something to note when I try to pull full articles later.

Let's first isolate the paper that is missing the year and take care of
that right now.
...
I found the paper! It was published in 2021. I don't have a precedent
for this. I'll discuss with my coauthors and put it in the amendment
document. I think that articles with missing information important for
analysis will have the information manually added directly from the
published article. If the paper with the missing data can't be located,
only then will it be dropped from analysis. I've noted this decision
change in amendment no. 11.

Now to find the papers missing keywords. (I don't have specific keyword
analysis but I might need this for the word relationship bit. Better to
have complete data than missing data.)
...
I'll save them all in a subfolder called manually_corrected. Then we'll
put revisions into these documents, save with corrected name, then
recombine the files together. That way we're not changing the raw
download file. Files will be saved as 'data needs X' and once the
corrections are in will be saved as 'data has X'.
...
NOTE: looking over the data that has a location, there are multiple
countries listed, so I'll need to clean up that data before I can do
location analysis on the first author location. I think if I slice up
the string to only include everything before a . or ; or if neither are
in a location cell, include the whole string. The . should catch most of
it as I've noticed they syntax seems to be a stop after listing the
country and before listing the next author's information.

It's really messy as sometimes names and emails and telephone numbers
are listed all in the same cell with the location. I'll probably have to
just isolate by country?

So let's load the files with corrected data and merge them. With the
addresses, I opened them in Excel to make pasting data easier, but it
changed the formatting on some content, so I deleted everything but the
key and location columns. I did the same with the other files just in
case opening the CSV in numbers changed any formatting.
...
To check that the merge is correct, let's see how many lines each
dataset has. When we merge we'll want the length to be the same
...
I combined the data by key, which means no new rows but creates new
columns (it changes the column names to end in .x and .y), and then
combine the data from both columns to a new column, and finally remove
the old columns that had NAs. It is not the best, but it works and is
easy to read.
...
Now to look at the number of articles by journal. I tried a density
ridgeline and it looks bad. Tried a binned one and it looks better but
still bad. Too many journals!

So some of the journals are not grouping well because of punctuation and
case sensitivity. I've got over 90 articles but some are clearly the
same like plos ONE and plos one. Let's tidy these up and try again. I'm
down to 82 but there are still some of the same journals with different
ways of presenting the name.

So the title content changes will be: - wiener tierärztliche
monatsschrift without the umlaut. - journal of applied animal welfare
science jaaws without the jaaws - journal of the american veterinary
medical association without the javma - animals with just the short
title - anthrozoos as one word

With this change we now have 78 unique journals that we can now analyse
...
Let's make a count of articles and see what percent of the total number
if papers we have.
...
# now add a column that is a percentage of 312 so we can see what percent of our data is from which journal
...
The top 5 out of 78 journals cover 148 (47.44%) articles, thus
containing less than half the dataset. Only two journals contained more
than 10% (\>31 articles) of the data. The first dog-specific journal
contained 7 (2.24%) articles and the first welfare-specific journal
contained 5 (1.6%) of articles. All articles with \>1% of the data were
English / non-translated Journals.
...
From the histograms, it looks like there's a boom around 2000. Let's
focus on these years.
...
Now that we've got some idea of what we're looking at, let's look at
publications by country and date. To do this we must parse out the
countries from the strings. We'll use countrycode to do this.

We need the code to read through the string, pull out complete countries
and make them as a list, then to select the first country to correspond
to the first author.

That said, a few things needed cleaning first. That includes removing
punctuation, putting to lowercase, and correcting the NAs that appeared
...
Pull the country name from a list of countries, then alter the location
data by removing punctuation and trying to correct some name variations
that aren't being identified or are otherwise being problematic.
...
Now we make a new table that is expanded so that every country found in
the string has its own row, such that two authors = two rows
...
After manual inspection of the entire dataset, some codes were still not
correct to the first author affiliated country. Here is a manual
correction of the affected country codes.
...
Finally we can plot the number or articles by country.
...
Let's manually check if if first author information is correct from a
subset...
...
Now to look at country and journal publication both. So look at
breakdown of publication within country. I selected all of the entries
labelled as Italy in the country and looked at the original location
column. All the first author locatinos are correct. And Italy was even
selected when the second authors were from a different country,
suggesting it correctly selected the first author location.

Then I did the same with UK, which is really important because it
required some data cleaning and the numbers kept changing as I cleaned
too. There was 1 error. All the others were accurate even when second
author was from another country. One of the data should have included
Saint Kitts and Nevis, which did show up in an earlier uncorrected
version. I'll see if I can fix this. Ross University School of
veterinary Medicine, Basseterre, St. Kitts, Saint Kitts and Nevis
...
It's good to tale a look at this but there are too many journals for
good information to come of it.
...
Now let's put this on a map! We want count by country connected to
countries with coordinates then put over an image of a map with
coordinates so we can make a visual. This code is based off the guide by
Dr Paul Christiansen available at <https://youtu.be/AgWgPSZ7Gp0>
...
As a reminder: Text mining was used to pull relevant words from
abstracts to explore: 

- frequency of word 
word frequency by year -
with correlation tests \* ON HOLD \* 
Sentiment analysis was used to -
separate words into sentiments, such as words preceded by 'not'
belonging to negative sentiments, and 
- word count by sentiment.

Analysis included - visualising a network of words used together and a
correlation of words including - a visualisation of correlations within
word clusters. 

Start here: <https://afit-r.github.io/tidy_text> End
here: <https://afit-r.github.io/word_relationships> <br> turn the
dataframe to tibble

As we will see below, there is a problem with combining words. Even when
stemming words, the UK vs US spelling of behaviour becomes a problem, as
the stems are still different. So I am also going to replace the
incidence of behav with behaviour. It won't really make sense in all
contexts, but since we're going to stem the words anyways this is needed
now.

First make a document with the content we want to look at and make
behaviour a word.
...
abstracts$abstract <- gsub("behav[^ ]*","behaviour",abstracts$abstract, ignore.case=TRUE) # UK spelling only
```

### Word Stemming
We take the abstracts and manipulate the strings so each word is a different row, remove alphanumerics, then turn words into their stem or word origin based off Snowball stemming.

```{r}
# rearrange the data for stemming
...
If we're rerunning anything, we can just load the saved stem word file here.
```{r}
tidy_stems <- read_csv("RayyanExport06022023/tidy_stem_words.csv")
```

Let's look at word frequency, being careful to count the number of
articles that contain a word, rather than number of times the word shows
up.

How many articles mention X word?
...
stem_word_count %>% 
  top_n(10)
```
There is more mention of behavoiur than study-- perhaps because study has synonyms like experiment, research, or project while behaviour doesn't. Not sure.
Behaviour is clearly the most common method/variable of interest. 21:23 are time, observ and stress. Which may be next, but given there are so many papers referencing behaviour (more than the n=62 limit), that would be the only measured variable we'll look at for the critical appraisal. 

### New Step: Standardise Words

Some words are the same but different tense, plural, etc. So I will need
to standardise the words to compare them effectively. We'll use Snowball
stemming as it has grown from contributions and so has a large
vocabulary.
...
```
This plot helps us get an idea for how words are used. The right skew suggests most words are not shared across the majority of articles, or rather that there are more unique words used in articles. This could indicate diversity in studied situations; if most of the articles used the same methods and situations, there might be more commonality in word usage. We aren't testing for this, but it would be an interesting thing for future investigation.

### Wordcloud

So it's hard to get the right cutoff without making the text hard to read. I've selected the top 20% stem words included in articles (n=62).
...
Now lets look at overall word frequency by year. 
...
Doing a pairwise count for words that are grouped together (occur in the
same article).
...
This was done just to check and see what is different between stemming and not stemming words. This is a consideration for discussion.
```{r}
...
Wait, I did this analysis on the article count not the article name? How
am I seeing which words correlate?
...
Let's try a circulr correlation jsut to see what we're dealing with
here.
...
And see what the top three most used methods are. Most used is not
frequency overall but articles that have included the word. This is very
broad and shallow - word might be used in reference but not an actual
method used. (will need to check that) We will use the articles pulled
and see which ones refer to a term but don't use it and can lightly
extrapolate from that Do sentiment analysis plot sentiments

## Top 3 Methods

End result: behaviour tops the chart here.
Okay so let's go back to our tidy_stems, sort by year in descending order and then filter for ones that contain behaviour?
...
I first sorted by year but month has a lot of missing data. Sorting by volume doesn't work, we'll chunk by year then once we hit 2020 we'll lookup and go by month? I'm sorting by article name for now. Will ask and we'll decide.
```{r}
...
Save the new file
I saved the top 80 in case there are irrelevant articles.
```{r}
